        ...
        %>
            <tr>
                ...
                <td>
                    ...
                    <% if (isLoggedIn) { %>
                        <span class="heart-icon <%= initialVotedByUser ? "heart-pink" : "heart-gray" %>" 
                              data-sake-id="<%= currentSakeId %>" 
                              data-voted="<%= initialVotedByUser ? "true" : "false" %>">
                            &#x2764;
                        </span>
                    <% } else { %>
                        <%-- ログインしていない場合はクリックイベントを捕捉するためにdata-sake-idを残し、スタイルでカーソルをデフォルトに --%>
                        <span class="heart-icon heart-gray" style="cursor: pointer;" 
                              data-sake-id="<%= currentSakeId %>" 
                              data-voted="false"> <%-- 常にfalseとしておく --%>
                            &#x2764;
                        </span>
                    <% } %>
                    <span class="likes-count" id="likes-count-<%= currentSakeId %>"><%= initialLikesCount %></span>
                </td>
            </tr>
            <%
            }
